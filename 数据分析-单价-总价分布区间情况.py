...
def calulate(filenames):
    ...
    for filename in filenames:
        ...
        # 处理 '单价' 列
        data['单价'] = data['单价'].replace('[,元/平]', '', regex=True).astype(float)

        # 处理 '总价' 列
        data['总价'] = data['总价'].replace('[,元/平]', '', regex=True).astype(float)
        for i in data['单价']:
            price = int(i)
            if price <= 20000:
                ydic['20000元以下'] += 1
        ...
        for i in data['总价']:
            ...
            if price <= 1000000:
                ...
                ydic2['1000万以上'] += 1
    draw_pie(list(ydic.keys()), list(ydic.values()), list(ydic2.keys()), list(ydic2.values()), '海淀二手房单价-总计分布图')
